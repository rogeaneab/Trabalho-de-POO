...
| **Emanuel Carvalho Belarmino** | Tratamento de erros, documentação e gestão do repositório — implementação do `GlobalExceptionHandler` e do registro padronizado `ErrorDetails`, exceções customizadas (`EstoqueInsuficienteException`, `RecursoNaoEncontradoException`), configuração do Swagger UI via Springdoc OpenAPI (`OpenApiConfig`), elaboração deste README e organização do repositório Git. |
